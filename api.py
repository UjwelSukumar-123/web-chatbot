...
def create_app(
    ...
    scraping_complete,
    target_website,
    set_scraping_in_progress,
    set_scraping_complete,
    set_target_website,
    set_scraped_data,
    set_structured_data,
    set_custom_data
    ...
    async def set_website_route(request_data: WebsiteRequest):
        try:
            ...
            if target_website != new_website:
                ...
                set_scraped_data([], [], None)
                set_structured_data([], None, [])
                set_scraping_complete(False)
                print("✅ Old data cleared")
            
            set_target_website(new_website)
    ...
    async def health_check():
        status = {
            ...
            "data_loaded": len(scraped_data_pages) > 0,
            "embeddings_created": scraped_data_embeddings is not None and (
                (hasattr(scraped_data_embeddings, 'shape') and scraped_data_embeddings.shape[0] > 0) if scraped_data_embeddings is not None else False
            ),
            "scraping_status": {
